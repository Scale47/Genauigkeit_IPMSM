%% Dipl.-Ing. Simon Roeschner (SR), 22.08.2022
%% Christoph Scale (CS), 03.09.2022
% Technische Universitaet Dresden
% Fakultaet Elektrotechnik und Informationstechnik
% Elektrotechnisches Institut
% Professur Elektrische Maschinen und Antriebe

%% Beschreibung
% Skript zur automatisierten Bestimmung von Induktivitaeten und Magnetverlusten (SR)/ Filterung des betrachteten Spektrums fuer verschiedene NoSegm/ Arbeitspunkte (CS) 
...
%       3. for-Schleife zur stueckweisen Begrenzung des Stromspektrums
%% Change-Log %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Datum %%%%%%% Kuerzel %% aenderungen %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 2022-08-22 %% SR     %% erstellt
% ---------- %% ------ %% --------------------------------------------------
% 2022-08-23 %% SR     %% Aliasing Fehler im Erstellen der 
%            %% SR     %% Spannungsfunktion behoben
% ---------- %% ------ %% --------------------------------------------------
% 2022-09-02 %% SR     %% Fehler bei Berechnung der Ersatzinduktivitaet
%            %% SR     %% behoben
% ---------- %% ------ %% --------------------------------------------------
% 2022-09-04 %% CS     %% Fehler bei uebergabe des Spektrums von Psi0 
...
% in Abhaengigkeit dieser Variable wird ein Index definiert um innerhalb der Tabelle APe.xlsx dann auf die benoetigten Parameter zuzugreifen 
Anzahl_AP = 1;
% 1. for-Schleife um die Arbeitspunktparameter festzulegen
for index_AP = 1:Anzahl_AP
    
% definieren eines Strings in Abhaengigkeit des index_AP um den Workspace passend zum AP abzuspeichern 
index_AP_str = num2str(index_AP+4);

% Festlegung des gewuenschten Formats zur Datumsangabe
formatOut = "mm_dd_yy";
% definieren einer Variable um den Workspace mit dem passendem Datum zu beschriften
d = datestr(now,formatOut);
% umwandeln der Datumsangabe in einen String um diese in der Bezeihnung des Workspace verwenden zu koennen
d_str = num2str(d);
...
grcParam.grcMaschine.grcPM.kdHoehe  = 3.8e-3;                              % Magnethoehe in [m]
grcParam.grcMaschine.grcPM.kdLaenge = 6.4e-3;                              % Axiale Magnetlaenge in [m]
grcParam.grcMaschine.grcPM.kdKappa  = 620e3;                               % Elektrische Leitfaehigkeit der Magnete in [S/m]
grcParam.grcMaschine.grcPM.kdMu     = 4*pi*1e-7 * 1.0503;                  % Permeabilitaet des Magneten in [Vs/Am]
grcParam.grcMaschine.zdGammaOffset  = 150;                                 % Verdrehen der Maschine zur Ausrichtung im dq-Koordinatensystem in [Grad], Version 18.2: 210

%% Definition von Zaehlern fuer die Arbeitspunktstabelle APe.xlsx um die jeweiligen Spalten der Tabelle zuzuordnen
Zaehler_n = 2;
Zaehler_Id_str = 3;
Zaehler_Iq_str = 4;
Zaehler_L_WK = 5;
...
grcAP.rgkdLWK   = L_WK;                                                     % Wickelkopfinduktivitaet in [H], aus Spalte RSPK*Ideff (R_SPK = Widerstand(?) Spulenkopf?)
...
grcAP.rgzdTheta = atan2(grcAP.rgkdIqstr, grcAP.rgkdIdstr) * 180/pi;         % Stromwinkel in der d-q-Ebene aus gegebenen Komponenten in [Grad]
grcAP.ziAPs     = length(grcAP.rgkdNn);
grcAP.rgkdLd    = L_d;                                                      % Laengsinduktivitaet in [H]
grcAP.rgkdLq    = L_q;                                                      % Querinduktivitaet in [H]
grcAP.rgkdL2    = nan(1,grcAP.ziAPs);                                       % Inversinduktivitaet in [H], muss noch ermittelt werden.
...
grcParam.grcFU.ziKk                 = 10;                                   % Anzahl der zu betrachtenden Traegerbaender
grcParam.grcFU.ziLmax               = 4;                                    % Maximale Anzahl betrachteter Seitenbaender pro Traegerband

%% Start von Maxwell (Zugriff auf Ansys Maxwell)
% Grab Ansys Electronics Application, Ansys Fenster oeffnet sich
...
%% Simulationsparameter fuer die Induktivitaetsbestimmung
ziAP = 1;
grcParam.grcGetL2.ziNu            = 20;                                                                 % Eingepraegte Oberschwingungsordnung fuer die Ermittlung der Invers-Reaktanz / - Induktivitaet
grcParam.grcGetL2.zdTeilPeriode   = 1;                                                                  % Anteil einer ganzen elektrischen Periode (Induktivitaeten), Ggf. Reduzierung der Simulation auf weniger als eine el. Periode (<=1)
grcParam.grcGetL2.ziSegmente      = 360;                                                                % Anzahl der Simulationsschritte (fuer Induktivitaeten), Ohne Zusatzschritt durch gleichen Ausgangs- wie Endzustand
grcParam.grcGetL2.kdInu           = 5;                                                                  % Amplitude des injizierten Signals zur Bestimmung der differentiellen Induktivitaet in [A]
grcParam.grcGetL2.zdKnu           = grcParam.grcGetL2.ziNu / grcParam.grcGetL2.zdTeilPeriode;           % Sollte durch den Kehrwert des zdTeilPeriode teilbar sein, sorgt dafuer, dass bei Verkuerzung der betrachteten Persodendauer, die gesuchte OS immer genauso viele Perioden hat, wie durch ihre Ordnung angegeben.
grcParam.grcGetL2.kdFnu           = grcParam.grcGetL2.zdKnu * grcAP.rgkdFfel(ziAP);                     % Frequenz des fuer die Induktivitaetsbestimmung ueberlagerten Signals in [Hz]
grcParam.grcGetL2.ziSchritte      = 2*grcParam.grcGetL2.ziSegmente * grcParam.grcGetL2.zdTeilPeriode;   % Induktivitaetsbestimmung: Anzahl der Segmente pro Pol, die die Luftspalttrennlinie besitzen muss, um mit den eingestellten Simulationsparametern das Clicking Mesh sicherzustellen, Verfeinerung ueber zdTeilPeriodeL2 moeglich

%% Definition aller Variablen und Funktionen fuer die Berechnung des Referenzwertes
%Erstellen eines "cell arrays" fuer die Daten des Simulationszeiten
Simulationszeit_Cell_100000_Hz = cell(1,1);
%Erstellen eines "cell arrays" fuer die Daten der Wirbelstromverluste
Wirbelstromverluste_gesamt_Cell_100000_Hz = cell(1,1);
%Erstellen eines "cell arrays" fuer die Daten der Wirbelstromverluste (Mittelwert)
Wirbelstromverluste_Mittelwert_Cell_100000_Hz = cell(1,1);

%% Definition aller Variablen und Funktionen fuer die Variation der NoS
%Erstellen einer temporaeren "table" fuer die Daten der Simulationen
temporaer_table = table();
%Erstellen eines "cell arrays" fuer die Daten des Simulationszeiten
Simulationszeit_Cell_2000_90000_Hz = cell(1,10);
%Erstellen eines "cell arrays" fuer die Daten der Wirbelstromverluste
Wirbelstromverluste_gesamt_Cell_2000_90000_Hz = cell(1,10);
%Erstellen eines "cell arrays" fuer die Daten der Wirbelstromverluste
Wirbelstromverluste_Mittelwert_Cell_2000_90000_Hz = cell(1,10);
%Erstellen eines "cell arrays" fuer die Daten der Wirbelstromverluste
Prozentuale_Abweichung_Simulation_Cell_2000_90000_Hz = cell(1,10);
%Erstellen eines "cell arrays" fuer die Daten des THD
THD_Cell_2000_90000_Hz = cell(1,10);
...
%% Sekanteninduktivitaeten mit d-Fluss und q-Fluss alleine
bSimLdLq = true;
if bSimLdLq
    ...
    sDummyMaterial  = '"SE N41MZ-GR 120deg dummy"'; % = Magnete ohne Koerzitivfeldstaerke (H_co = 0 A/m). --> Ermittlung von L_d und L_q
    ...
    %% Erregung setzen (sinusfoermig mit injizierter Oberschwingung)
    ...
    %% Ausgabe der d- und q-Sekanteninduktivitaeten
   ...
   %% ueberschreiben der Werte
     % Induktivitaet in d-Richtung in [H]
    grcAP.rgkdLd(ziAP) = mean(tblLdLq.rgkdLd);   
     % Induktivitaet in q-Richtung in [H]
...
%% Vormagnetisierung wieder hinzufuegen (entsprechendes Material zuweisen)
sMagnetNames = 'Magnet_1,Magnet_2,Magnet_3,Magnet_4,Magnet_5,Magnet_6';
% = Magnete mit Koerzitivfeldstaerke (H_co = -886 kA/m)
...
sIOSFunktion = ' + kdInu*cos(2*pi*kdFnu*time)';                            % Zeitfunktion Oberschwingungskomponente, fuer alle drei Phasen gleich, damit Nullsystem sichtbar wird. Aus dem Strom im Nullsystem wird spaeter eine Induktivitaet bestimmt.
rgsWindingGroupCurrent = {
...
%% Berechnung der Ersatzinduktivitaet
...
% L2"-Spektrum betrachten
plot(rgkdL2); 
% Gegenlaeufige Induktivitaet in [H]
...
%clear tblNullsystem grcOpts grcEvalL
    ...
    %Erstellung eines Strings um die Benneung des Workspace mit den aktuellen NoS zu ergaenzen
    NoS_str= num2str(NoS);

    %% Zeigerdiagramm nachrechnen (vgl. Bericht5, S. 36)
    kdOmega = 2*pi*grcAP.rgkdFfel(ziAP);
    % Spannungsabfall in der q-Achse (= Imaginaerteil der Strangspannung)
    ...
    %% Initialsierung: Auslesen der benoetigten Groessen
    bStern            = grcParam.grcMaschine.bStern;                            % ist in Stern geschaltet?
    zdGammaOffset     = grcParam.grcMaschine.zdGammaOffset;                     % Verdrehen der Maschine zur Ausrichtung im dq-Koordinatensystem in [Grad]
    kdUzk             = grcParam.grcFU.kdUzk;                                   % Zwischenkreisspannung in [V]
    ziKk              = grcParam.grcFU.ziKk;                                    % Anzahl der zu betrachtenden Traegerbaender
    kdFfc             = grcParam.grcFU.kdFfc;                                   % Schaltfrequenz des Umrichters in [Hz]
    ziLmax            = grcParam.grcFU.ziLmax;                                  % Maximale Anzahl betrachteter Seitenbaender pro Traegerband
    ...
    zdTheta           = grcAP.rgzdTheta(ziAP);                                  % Stromwinkel in der d-q-Ebene aus gegebenen Komponenten in [Grad]
    kdFfel            = grcAP.rgkdFfel(ziAP);                                   % Elektrische Grundschwingungsfrequenz in [Hz]
    kdLd              = grcAP.rgkdLd(ziAP);                                     % Induktivitaet in d-Richtung in [H]
    kdLq              = grcAP.rgkdLq(ziAP);                                     % Induktivitaet in q-Richtung in [H]
    kdL2              = grcAP.rgkdL2(ziAP);                                     % Gegenlaeufige Induktivitaet in [H]
    kdLWK             = grcAP.rgkdLWK(ziAP);                                    % Zusatzinduktivitaet in [H] zur Beruecksichtigung des magn. Streuung im Wickelkopf
    sRstrModel        = 'quad';                                                 % Berechnungsmodell fuer frequenzabhaengigen Strangwiderstand

    %% Betrachtete Zeitachse
    kdT1     = 1/kdFfel; % Grundschwingungsperiode
    ziPunkte = 2; % Anzahl der Punkte pro Periode des hoechstfrequenten Signals (mindestens Nyqvist = 2), bei hoeherer Zahl wird das Spektrum auf der Frequenzachse einfach nur laenger.
    ...
    rgkdT    = rgkdT(1:end-1); % Der Punkt bei kdT1 ( = Anfang der naechsten Periode) wird erst nach der Phasenanpassung hinzugefuegt, um Knicke im Verlauf zu vermeiden.
    ...
    %% Verlaeufe der Leiter-Mittelpunktspannungen am Wechselrichterausgang je Phase berechnen
    % Theoretische Gleichung nach Bernet: "Selbstgefuehrte Stromrichter am Gleichspannungszwischenkreis", S. 108f
    % Falls andere Grundgleichungen/Pulsmuster/Modulationsverfahren genutzt werden sollen, muss dieser Block hier geaendert werden.
    ...
    ziKBandMax  = kdFfg / kdFfc;        % Max. Anzahl der Traegerbaender, durch Frequenzwahl der Eingangsparameter festgelegt //CS:wozu diese Berechnung...wird durch "ziKk" festgelegt? Weil es bei ziLmax von 4 ab bei f_el>333.3Hz Sinn macht da dann groesser als f_c=1000
    rgzdPhiU = [    % CS: Erstellung der 3 Leiter durch Phasenverschiebung
        zdPhiUum;           % CS: Uum
        zdPhiUum - 2*pi/3;  % CS: Uwm
        zdPhiUum + 2*pi/3;  % CS: Uvm
        ];
    % CS: GRUNDSCHWINGUNG, Spannungswerte werden Stueck fuer Stueck aufaddiert in Abhaengigkeit des Zeitvektors
    rgkdUm = rgkdUm + zdMod/2 * cos(2*pi*kdFfel*rgkdT + rgzdPhiU); 

    % ueber alle Traegerbaender iterieren
    for ziK = 1:ziKBandMax 
        % Berechnung erfolgt bezogen auf die Zwischenkreisspannung
        % CS: aufaddieren aller OBERSCHWINGUNGEN aus Traegerbandern
        rgkdUm = rgkdUm + 2/(pi*ziK) .* besselj(0,(ziK*pi*zdMod/2)) .* sin(ziK*pi/2) .* cos(ziK*2*pi*kdFfc.*rgkdT);
        % alle Seitenbaender pro Traegerband betrachten
        for ziL = [ -ziLmax:-1 , 1:ziLmax ] 
            rgkdUm = rgkdUm + 2/(pi*ziK) .* besselj(ziL,(ziK*pi*zdMod/2)) .* sin((ziK+ziL)*pi/2) .* cos(ziK*2*pi*kdFfc.*rgkdT + ziL*(2*pi*kdFfel.*rgkdT+rgzdPhiU)); % CS: aufaddieren aller OBERSCHWINGUNGEN aus Seitenbaendern
        end
    end
    ...
    rgkdUstrSpektrum      = fft(rgkdUstr,length(rgkdUstr),2) / length(rgkdUstr) * 2; % *2 = "Rueberklappen" der Amplituden der negativen Frequenzen // CS: gibt die Fourier-Transformation entlang der Dimension dim zurueck. Wenn X beispielsweise eine Matrix ist, gibt fft(X,n,2) die n-Punkt-Fouriertransformation jeder Zeile zurueck
    ...
    % (Beruecksichtigt frequenzabhaengigen Strangwiderstand, interpoliert aus Simulationsergebnissen
    % und die Ersatzinduktivitaet fuer das Oberschwingungsverhalten aus den Vorgaengersimulationen)
    ...
    for ziNu = 3:length(rgkdUstrSpektrum) % nur Oberschwingungen ohne Grundschwingung durchgehen (1 = Gleichanteil, 2 = Grundschwingung)
        rgkdZSpektrum(1, ziNu) = fcnInterpolRstr(kdFfel*(ziNu-1), sRstrModel) + 1j * 2*pi*kdFfel*(ziNu-1) * (kdLWK);    % HIER UNBEDINGT NOCH kdL2 HINZUFueGEN
    end
    plot(rgkdFf(1:end-1),abs(rgkdZSpektrum(1,:))); xlabel("Frequenz in Hz"); ylabel("Impedanz in Ohm"); % Spektrum Test

    %% Berechnung des komplexen Stromspektrums ueber die Impedanz
    ...
    %% Anpassung des Stromspektrums basierend auf der vorgegebenen Stromgrundschwingung und den arbeitspunktabhaengigen Induktivitaeten
    kdIest = kdULL1peak/(sqrt(3)*2*pi*kdFfel*(kdLd + kdLq)/2); % Mittlerer Grundschwingungsstrom, der bei Annahme des Mittelwerts aus Ld und Lq und Vernachlaessigung des Strangwiderstands fliessen wuerde. Grundidee ist, dass die Ersatzinduktivitaet fuer das Oberschwingungsverhalten nicht auf die Grundschwingung anwendbar ist und fuer diese naeherungsweise der Mittelwert aus Ld und Lq angenommen werden kann.
    ...
    rgkdIstrSpektrum(:,3:end) = rgkdIstrSpektrum(:,3:end) .* zdIskal; % Runterskalieren des Stromspektrums bezogen auf tatsaechlichen Grundschwingungsstrom.
    ...
    % mit diesem Skalierungsfaktor kann bei gegebener Frequenz f_Filter die Spalte festgelegt werden, ab der die Tabelle "rgkdIstrSpektrum" mit dem Wert 0 aufgefuellt werden soll, so wird das Spektrum begrenzt
    ...
    %end 
        ...
        for index2 = 1:6
        % Ermittlung der Spalte ab der die Tabelle "rgkdIstrSpektrum" mit dem Wert 0 aufgefuellt werden soll    
        y= f_Filter/x;
        % auffuellen der tabelle "rgkdIstrSpektrum" mit dem Wert 0
        ...
        %% Ruecktransformation der Stromspektren
        rgkdIstr = zeros(3, ziDeltaT); % Preallocation
        ...
        %% Phasenverschiebung der Strangstroeme im Modell (in Grad)
        %zdDeltaPhiI   = zdGammaOffset - (180 - zdTheta); % Phasenverschiebung des Strangstromes im Modell, ist 180Grad-dq-Winkel, da der Standardfall von reinem NEGATIVEM d-Strom ausgeht, ausserdem abhaengig vom Arbeitspunkt(?)
        zdDeltaPhiI   = zdGammaOffset + zdTheta; % Phasenverschiebung des Strangstromes im Modell, abhaengig vom Arbeitspunkt
        ...
        %% Hinzufuegen des letzten Punktes (der wieder dem ersten entspricht) zum Auffuellen der gesamten Periodendauer (Vermeidung eines off-by-one-errors) und Knicken im Verlauf (diese koennen das Gesamtergebnis signifikant beeinflussen)
        rgkdT(end+1)      = kdT1;
        ...
        %% Stromverlaeufe importieren und einbinden
        mwEditCurrentDatasets(oDesign, grcExport);

        %% Erregung setzen (importierte Stromverlaeufe)
        oModule = oDesign.GetModule('BoundarySetup');
        rgsPhasen = [
            "PhaseU";
            "PhaseV";
        ...
        % Design analysieren
        oDesign.Analyze('Setup2'); % mit SaveFields, brauchen wir zum Bestimmen der Verluste der Einzelmagneten, Gesamtmagnete ueber SolidLosses.
        % Stoppen der Simulationszeit
        StopZeit = toc(tiSimStart); 
        fprintf("AP%u: Simulation mit %u Segmenten und einer Frequenz von %u nach %.2f min fertig. \n",index_AP+4, NoS,f_Filter, StopZeit/60);

        %% Referenzwert abspeichern
        if index2 == 1
        %% Ergebnis aus Maxwell exportieren
        oModule = oDesign.GetModule('ReportSetup');
        oModule.UpdateReports(["Wirbelstromverluste_gesamt","Wirbelstromverluste_Mittelwert"]); % Reports updaten (nicht während der Simulation, erst am Schluss!)
        ...
        % Einlesen der Daten aus gewaehltem .csv File in MATLAB Tabelle
        temporaer_table = table(StopZeit);
        % Bezeichnung der Spalten geeignet aendern
        temporaer_table.Properties.VariableNames = "Simulationszeit_s_"+f_Filter_str;
        % abspeichern der Daten jedes Simulationsdurchlaufes der Variabel "Drehmoment" in einer neuen Zelle (spaltenweise) in Abhaengigkeit des Index
        Simulationszeit_Cell_100000_Hz(:,1) = {temporaer_table};
        ...
        % abspeichern der Daten jedes Simulationsdurchlaufes der Variabel "Drehmoment" in einer neuen Zelle (spaltenweise) in Abhaengigkeit des Index
        Wirbelstromverluste_gesamt_Cell_100000_Hz(:,1) = {temporaer_table};
        ...
        % abspeichern der Daten jedes Simulationsdurchlaufes der Variabel "Drehmoment" in einer neuen Zelle (spaltenweise) in Abhaengigkeit des Index
        Wirbelstromverluste_Mittelwert_Cell_100000_Hz(:,1) = {temporaer_table};
        ...
        temporaer_table.Properties.VariableNames = "Simulationszeit_s_"+f_Filter_str;
        % abspeichern der Daten jedes Simulationsdurchlaufes der Variabel "Drehmoment" in einer neuen Zelle (spaltenweise) in Abhaengigkeit des Index
        Simulationszeit_Cell_2000_90000_Hz(:,index2) = {temporaer_table};
        ...
        % abspeichern der Daten jedes Simulationsdurchlaufes der Variabel "Drehmoment" in einer neuen Zelle (spaltenweise) in Abhaengigkeit des Index
        Wirbelstromverluste_gesamt_Cell_2000_90000_Hz(:,index2) = {temporaer_table};
        ...
        % abspeichern der Daten jedes Simulationsdurchlaufes der Variabel "Drehmoment" in einer neuen Zelle (spaltenweise) in Abhaengigkeit des Index
        Wirbelstromverluste_Mittelwert_Cell_2000_90000_Hz(:,index2) = {temporaer_table};
        ...
        % abspeichern der Daten jedes Simulationsdurchlaufes der Variabel "Drehmoment" in einer neuen Zelle (spaltenweise) in Abhaengigkeit des Index
        THD_Cell_2000_90000_Hz(:,index2) = {temporaer_table};

        %% Berechnung der prozentualen Abweichung der Mittelwerte (Wirbelstromverluste) innerhalb einer Iteration
        Wert_Eins = Wirbelstromverluste_Mittelwert_Cell_2000_90000_Hz{:,index2};
        Iteration_Eins = Wert_Eins{:,2};
        Iteration_Zwei = Wirbelstromverluste_Mittelwert_Cell_100000_Hz{1,1}{1,2};
        ...
        % abspeichern der Daten jedes Simulationsdurchlaufes der Variabel " Prozentuale_Abweichung_Simulationen" in einer neuen Zelle (spaltenweise) in Abhaengigkeit des Index
        Prozentuale_Abweichung_Simulation_Cell_2000_90000_Hz(:,index2) = {temporaer_table};
        %Ausgabe der aktuellen Abweichung
        fprintf("AP%u: Die aktuelle Abweichung einem einem Spektrum von %u Hz zu %d Hz betraegt %.2f%%. \n",index_AP+4, f_Filter, 103000, Prozentuale_Abweichung_Simulationen);
        filename = "AP"+index_AP_str+"_NoS_"+NoS_str+"_Simulation_"+d_str+"_Feinmodell.mat";
        save(filename)
        %bereinigt den Ordner IPMSMs.aedtresults
       
        end
        
        %% verringern des Frequenzwerts, auf den das Spektrum begrenzt werden soll
        if index2 == 1
           f_Filter = f_Filter - 60000; 
        end
        
        if index2 > 1
        if rem(index2,2)
            f_Filter = f_Filter - 13000; 
        else
            f_Filter = f_Filter - 7000;
        end
        
        %plot(rgkdFf(1:end-1),abs(rgkdIstrSpektrum(1,:))); xlabel("Frequenz in Hz"); ylabel("Stromamplitude in A"); % Spektrum Test

        % Ende der Schleife zur Abspeicherung der Daten und Berechnung der prozentualen Abweichung
        end 
    
        %Ende for Schleife fuer Anpassung Stromspektrum
        end

    %% Abspeichern der Daten in txt Dateien/ Abspeichern der Werte eines Arbeitspunktes
    
    %% Definieren aller Variablen und Funktionen
    Anzahl_durchgefuehrter_Simulationen = index2;
    f_Filter_Benennung = 43000;
    ...
    Ordner_Verlauf_Wirbelstromverluste = "C:\Users\scale\Desktop\Daten Umrichterspeisung";
    Ordner_Verlauf_Simulationszeit_Prozentuale_Abweichung = "C:\Users\scale\Desktop\Daten Umrichterspeisung";
    Ordner_Daten_Simulationszeit_Abweichung = "C:\Users\scale\Desktop\Daten Umrichterspeisung";
    ...
    f_Filter_Benennung_Cell = cell(1,1);
    f_Filter_Benennung_Cell{1,1} =  f_Filter_Benennung;
    Tabelle_f_Filter= table();
    Tabelle_f_Filter(1,1)= f_Filter_Benennung_Cell(1,1);
    
    f_Filter_Benennung_str = num2str(f_Filter_Benennung);
    ...
    Tabelle_Verlauf_Wirbelstromverluste = Wirbelstromverluste_gesamt_Cell_2000_90000_Hz{:,index_zwei+1};
    ...
    table_path_format_Verlauf_Wireblstromverluste = fullfile(Ordner_Verlauf_Wirbelstromverluste, "AP"+index_AP_str+"_"+f_Filter_Benennung_str+"_Wirbelstromverluste_Verlauf_"+NoS_str+".txt");
    %ausgeben der Tabelle fuer die Prozentuale Abweichung als Textdatei fuer jede einzelne Simulation in einer seperaten Textdatei
    ...
    Tabelle_Cell_Extrahierung_Indexierung = table(Prozentuale_Abweichung_Simulation_Cell_2000_90000_Hz{:,index_zwei+1});
    ...
    Tabelle_NoS_Prozentuale_Abweichung(index_zwei,1) = Tabelle_f_Filter(1,1);
    ...
    %% Simulationszeit
    %definieren aus welchem "cell" Daten extrahiert werden sollen
    Tabelle_Cell_Extrahierung_Indexierung = table(Simulationszeit_Cell_2000_90000_Hz{:,index_zwei+1});
    %Zugreifen auf Daten mehrere Ebenen tief: 
    %https://de.mathworks.com/help/matlab/matlab_prog/multilevel-indexing-to-access-parts-of-cells.html
    %zugreifen auf Wert in cell(1,1) mit {1,1} (in dem Fall eine Tabelle) und dann zugreifen auf den Wert innerhalb der Tabelle mit {1,1}{1,1}
    %abspeichern dieses extrahierten Wertes in einer neuen Tabelle
    ...
    Tabelle_NoS_Simulationszeit(index_zwei,1) = Tabelle_f_Filter(1,1);
    ...
    %% THD
    %definieren aus welchem "cell" Daten extrahiert werden sollen
    Tabelle_Cell_Extrahierung_Indexierung = table(THD_Cell_2000_90000_Hz{:,index_zwei+1});
    %Zugreifen auf Daten mehrere Ebenen tief: 
    %https://de.mathworks.com/help/matlab/matlab_prog/multilevel-indexing-to-access-parts-of-cells.html
    %zugreifen auf Wert in cell(1,1) mit {1,1} (in dem Fall eine Tabelle) und dann zugreifen auf den Wert innerhalb der Tabelle mit {1,1}{1,1}
    %abspeichern dieses extrahierten Wertes in einer neuen Tabelle
    ...
    Tabelle_Table_Extrahierung_round = round(Tabelle_Table_Extrahierung{1,1},3,"significant");
    %abspeichern der prozentualen Abweichung in einer Tabelle zusammen mit der Simulationszeit (siehe unten) zur Darstellung in LaTeX
    Tabelle_THD(index_zwei,1) = Tabelle_f_Filter(1,1);
    Tabelle_THD(index_zwei,2) = table(Tabelle_Table_Extrahierung_round(1,1));
    
    %if index_zwei == 1
     %   f_Filter_Benennung = f_Filter_Benennung - 60000; 
    %end
    
   
    if rem(index_zwei,2)
        f_Filter_Benennung = f_Filter_Benennung - 7000;
    else
        f_Filter_Benennung = f_Filter_Benennung - 13000;
    end
    ...
    %ausgeben der Tabelle fuer die Prozentuale Abweichung (y-Achse) ueber der Simulationszeit (x-Achse) als Textdatei
    ...
    % Ende for Schleife fuer Anpassung NoS
    end
